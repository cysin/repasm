...
char *ErrString(int err)
{
	...
	case ERROR_AVI_FILE:
		return "AVI file (renamed MPQ)";
	case ERROR_NOT_SUPPORTED:
		return "MPQ version not supported";
    default:
...
HANDLE CreateFile(const char *sFileName, DWORD ulMode, DWORD ulSharing, void *pSecAttrib, DWORD ulCreation, DWORD ulFlags, HANDLE hFile)
{
	...
	case CREATE_ALWAYS:
    case CREATE_NEW:
			return (HANDLE)open(sFileName, O_RDWR | O_CREAT | O_TRUNC, S_IRWXU | S_IRWXG | S_IROTH);
    default:
...
{
	if (hFile == INVALID_HANDLE_VALUE) // Fix by Taiche : removed the hFile == NULL test because the CreateFile function above can return a HANDLE equal to 0 WHICH IS A LEGAL VALUE and does not mean the handle is NULL.
	{
        return 0xffffffff;
	}
	...
	*ulOffSetHigh = 0; // Will not support files > 2GB, unlike StormLib for Windows
...
DWORD SetFilePointer(HANDLE hFile, LONG lOffSetLow, LONG *pOffSetHigh, DWORD ulMethod)
{
	if(pOffSetHigh != NULL)
	{
		DWORD ret = lseek64((int)hFile, (off64_t)(*pOffSetHigh) << 32 | (DWORD)lOffSetLow, ulMethod);
		return ret;
	}

	return lseek64((int)hFile, (DWORD)lOffSetLow, ulMethod);
}
